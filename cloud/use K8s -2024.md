...
```
> NodePort范围在 30000-32767 之间，ip是外部机器的ip
...
统一的网关入口
ingress对service 进行了域名映射 my-app.default.svc -> hostname:port
...
spec:
  ...
  - host: "hg.com"
    http:
      paths:
      - pathType: Prefix
        path: "/hello(/|$)(.*)" -- 指的是第一个参数不要，第二个参数保留， /hello/abc/efg -> /hello/efg
        backend:
          service:
            name: helloapp
            port:
